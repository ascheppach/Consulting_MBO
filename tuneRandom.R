...
### MBO Prozess
data2 = read.csv(text = "power,time,gas,pressure,ratio
...
r <- with(data2, which(data2$gas=="Argon", arr.ind=TRUE))
newdata2 <- data2[r, ]
data2 <- newdata2
data2$gas <- NULL

## execute
model2 = train(makeLearner("regr.km", nugget.estim = TRUE, control = list(trace = FALSE)), makeRegrTask(data = data2, target = "ratio"))
...
# Execute the entire mbo-process n times (n=number of experiments/iterations) and save the resuls in a list named tuneResult
tuneRandom = function(Experiments) {
  
  if (Experiments[[1]] == "regr.km") {
    lrn = makeLearner("regr.km", predict.type = "se", nugget.estim = TRUE, covtype = Experiments[[2]]) 
  }
  ...
  if (Experiments[[4]] == "makeMBOInfillCritEIcontrolExploration") {
    ctrl = setMBOControlInfill(ctrl, opt = "focussearch", opt.focussearch.maxit = 20, opt.focussearch.points = 5, crit = makeMBOInfillCritEIcontrolExploration(controlExploration = Experiments[[5]]))
  }
  
  if (Experiments[[4]] == "makeMBOInfillCritAdaEIctrlExploration") {
    ctrl = setMBOControlInfill(ctrl, opt = "focussearch", opt.focussearch.maxit = 20, opt.focussearch.points = 5, crit = makeMBOInfillCritAdaEIctrlExploration(controlExplorationStart = Experiments[[6]], controlExplorationEnd = Experiments[[7]]))
  }
